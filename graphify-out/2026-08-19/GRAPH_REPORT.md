# Graph Report - minimacore-desktop  (2026-08-19)

## Corpus Check
- 104 files · ~276,627 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2265 nodes · 5176 edges · 118 communities (105 shown, 13 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 505 edges (avg confidence: 0.61)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0c5131df`
...
Cohesion: 0.08
Nodes (57): derivePools(), done(), finishScan(), fund(), gatherOwned(), gatherRegistry(), group(), parseScripts() (+49 more)

### Community 4 - "webwallet.js"
Cohesion: 0.08
Nodes (53): claim(), coinAmount(), deleteTxn(), grain(), loadKeys(), lock(), lockFromCoins(), maybeGrain() (+45 more)
...
### Community 89 - "atomix/service.js"
Cohesion: 0.56
...
Cohesion: 0.14
Nodes (14): Cross-platform Builds (0.13.0), Per-address WOTS Key-Uses Safety Checker (0.7.1), Web Wallet (MegaMMR-gated wallet-from-seed), Cross-platform Build Matrix (mac/win/linux), Bundled JRE via jlink, Desktop Build GitHub Actions Workflow, Bundled JRE (no system Java needed), First-run Node Wizard (+6 more)
...
### Community 99 - "ethwallet-unit.js"
Cohesion: 0.29
...
## Knowledge Gaps
- **355 isolated node(s):** `{ app, BrowserWindow, ipcMain, dialog, Tray, Menu, nativeImage, shell, Notification, session, clipboard }`, `path`, `fs`, `config`, `node` (+350 more)
...
- **Why does `p()` connect `p` to `peg.js`, `casino.js`, `pandapools/service.js`, `s`, `showConfirm`, `k`, `withTimeout`, `casinoActivity`, `atomix.js`, `poolmgr.js`, `"node_modules/tweetnacl/nacl-fast.js"`, `ethTokensLoad`, `rpcCall`, `init`, `ethSendReview`, `"node_modules/bn.js/lib/bn.js"`, `vestr.js`, `d`?**
  _High betweenness centrality (0.203) - this node is a cross-community bridge._
- **Why does `Renderer App Shell (header + tab navigation + views)` connect `Renderer App Shell (header + tab navigation + views)` to `app.js`, `tokenicons.js`, `minimaCore Desktop App`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `k()` connect `k` to `store.js`, `pandapools/service.js`, `ui.js`, `curve.js`, `"node_modules/tweetnacl/nacl-fast.js"`, `renderSettings`, `history.js`, `atomix/service.js`, `config.js`, `d`?**
...
- **What connects `{ app, BrowserWindow, ipcMain, dialog, Tray, Menu, nativeImage, shell, Notification, session, clipboard }`, `path`, `fs` to the rest of the system?**
