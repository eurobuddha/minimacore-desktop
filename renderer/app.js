...
function onStatus(s) {
  ...
  if (running && (waitingForNode || !wasRunning)) {
    ...
    startRefresh();
    // fresh node on a real network still needs its wallet seed set up (new or restore); solo keeps its own.
    if (!CFG.walletDone && CFG.network !== "solo") { showWalletSetup(); }
    else { hideSetup(); renderActive(); }
  }
...
function showSetup() {
  ...
  box.innerHTML = `
    <div class="view__desc">Choose how your node connects. You can change this later in the Node tab.</div>
    <label class="opt sel" data-net="mainnet"><b>Mainnet</b><span>Join the live Minima network (syncs the real chain).</span></label>
    ...
    <input class="field__input" id="customPeer" placeholder="host:port" style="display:none;margin-top:2px" />
    <div class="adv" id="advToggle">▸ Advanced</div>
    <div id="advBox" style="display:none">
      ...
      <label class="opt" id="megammr" style="display:flex;align-items:center;gap:8px">
        <input type="checkbox" ${CFG.megammr ? "checked" : ""}/> <span style="margin:0;color:var(--text)">Full history (megammr) — bigger, slower initial sync</span></label>
    </div>
    <button class="btn btn--primary btn--full" id="startNode" style="margin-top:10px">Start node</button>`;
  ...
  el("advToggle").onclick = () => {
    const b = el("advBox"), on = b.style.display === "none";
    b.style.display = on ? "" : "none"; el("advToggle").textContent = (on ? "▾" : "▸") + " Advanced";
  };
  ...
  el("startNode").onclick = async () => {
    ...
    const patch = {
      setupDone: true, network: net, basePort,
      dataFolder: el("dataFolder").value || "",
      megammr: el("megammr").querySelector("input").checked,
      customConnect: net === "custom" ? (el("customPeer").value || "") : "",
      peers: net === "mainnet" ? CFG.megammrHost : ""   // the -megammr node serves p2p bootstrap too
    };
    if (net === "custom" && !patch.customConnect) { toast("Enter a host:port for the custom peer."); return; }
    el("startNode").textContent = "Starting…"; el("startNode").disabled = true;
...
// ---- wallet seed onboarding (new / restore) — real networks only -----------
function seedFrom(v) { return v && (v.phrase || v.seedphrase || v.seed || v.mnemonic) || ""; }
function showWalletSetup() {
  el("shell").style.filter = "blur(2px)";
  const box = el("setupBody");
  box.innerHTML = `
    <div class="view__desc">Set up the seed that controls your funds. It never leaves this Mac.</div>
    <label class="opt sel" data-w="new"><b>New wallet</b><span>Generate a fresh seed on this node — back up the 24 words, they're the only way to recover your funds.</span></label>
    <label class="opt" data-w="restore"><b>Restore from seed</b><span>Import an existing 24-word seed and fast-sync via MegaMMR (seconds).</span></label>
    <div id="wbody"></div>`;
  let mode = "new";
  const paint = () => { box.querySelectorAll(".opt[data-w]").forEach(o => o.classList.toggle("sel", o.dataset.w === mode)); wbody(mode); };
  box.querySelectorAll(".opt[data-w]").forEach(o => o.onclick = () => { mode = o.dataset.w; paint(); });
  paint();
  el("setup").style.display = "";
}
function wbody(mode) {
  ...
  if (mode === "new") {
    b.innerHTML = `
      <div class="view__desc" style="margin-top:8px">Reveal your seed and write it down before continuing.</div>
      <button class="btn btn--outline btn--full" id="revealSeed">Reveal my seed phrase</button>
      ...
      <button class="btn btn--primary btn--full" id="newGo" style="display:none">Finish — sync to the network</button>`;
    ...
    el("newGo").onclick = async () => {
      ...
      el("newGo").disabled = true; el("newGo").textContent = "Syncing…";
      try { await cmd(`megammrsync action:resync host:${CFG.megammrHost}`); } catch (e) { /* host-only tip sync; non-fatal */ }
      CFG = await api.saveConfig({ walletDone: true });
      hideSetup(); renderActive(); toast("Wallet ready ✓", "ok");
    };
  } else {
    b.innerHTML = `
      <div class="field" style="margin-top:8px"><div class="field__label">Your 24-word seed phrase</div>
      ...
      <button class="btn btn--primary btn--full" id="rGo">Restore + sync</button>`;
    el("rGo").onclick = async () => {
      const seed = el("rSeed").value.trim().replace(/\s+/g, " ");
      const keyuses = parseInt(el("rKeyuses").value, 10);
      const host = el("rHost").value.trim() || CFG.megammrHost;
      ...
      el("rGo").disabled = true; el("rGo").textContent = "Restoring + syncing…";
      try {
      ...
      } catch (e) { toast("Restore failed: " + e.message, "err"); el("rGo").disabled = false; el("rGo").textContent = "Restore + sync"; }
    };
  }
}
...
async function renderSettings() {
  ...
  el("setRestore").onclick = () => showWalletSetup();
